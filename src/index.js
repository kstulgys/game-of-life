...
class App extends Component {
	...
	play = () => {
		...
		const conditions = (col, cidx, ridx, cg) => {
			// const result = withFirstState && withFirstState(cg[ridx][cidx])
			// const defineEqual = start => next => (start === next ? true : false)
			// const withFirstState = defineEqual(cg[ridx][cidx])
			let count = 0
